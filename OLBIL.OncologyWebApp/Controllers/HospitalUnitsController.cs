...
namespace OLBIL.OncologyWebApp.Controllers
{
    ...
    {
        [HttpGet]
        public async Task<ActionResult<ListModel<HospitalUnitModel>>> GetAll([FromQuery]GetHospitalUnitsListQuery request)
        {
            return Ok(await Mediator.Send(request));
        }

        [HttpGet("search")]
        public async Task<ActionResult<ListModel<HospitalUnitModel>>> Search(string searchTerm)
        {
